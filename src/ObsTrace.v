...
Module ObsTrace (M:Machine).
  ...
  | ObsEqNow : forall o OO OO'
                      (HR : (*R*)ObsTraceEq OO OO' : Prop),
      ObsTraceEq(*_gen R*) (notfinished (Out o) OO) (notfinished (Out o) OO')
  (*
 ...
 *)
  ...
  Proof.
    ...
    (* Try by inversion. *)
    - destruct a'.
      + destruct (EqOut_tauN O1' o1' Last' o T Eq) as [n [HTau | [T' HTau]]](*;
        pfold*). 
        * rewrite HTau in *.
          clear HTau.
...
Proof.
  ...
  rewrite idTrace_eq. pattern (ObsOfM (RunOf m)) at 1.
  rewrite idTrace_eq.  simpl.
  destruct (mpstep (m,p)); simpl.
  - destruct p0; simpl.
