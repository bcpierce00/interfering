...
\leo{Rough story of the paper based on the discussion at POPL:}
\begin{itemize}
\item Noninterference is a desirable security property: secret information
  does not affect pulbic outputs of systems.
\item Traditionally, it is expressed as a {\em hyperproperty}, a
  property over pairs of execution traces. \leo{Two machines, differ
    secret data, run, indist}
\item The two execution traces are allowed differ arbitrarily in their
  control flow: branching on secret data can cause the machines to follow
  different execution paths.
\item \leo{Why is that bad? Hard to test/visualize? Even though interesting and permissive?}
\item \leo{Where do we mention contours, if at all?}
\item 
\item In this paper we describe a restricted form of hyperproperties:
  lockstep-hyperproperties, where the two traces execute the same
  sequence of instructions.
\item This kind of hyperproperty is still of practical interest: we
  can use it to encode both taint tracking, heap safety, and stack
  safety as noninterference properties.
\item Intuitively, taint tracking enforces noninterference until the
  program branches on tainted data. Then all bets are off.
\item On the other hand, stack safety is a monitoring policy that can
  enforce this lockstepness.
\item By restricting the class of hyperproperties we care about, we
  are also able to give a pretty extensional characterization of
  well-bracketed classification and declassification: what is
  considered secret can change throughout the execution.
\item For example, stack safety caller/callee accessible data.  
\end{itemize}

Contributions:
\begin{itemize}
\item \leo{Something about lockstep-hyperproperties as an abstraction}
\item An extensional characterization of taint tracking.
\item A novel stack safety policy and property characterized as noninterference.
\item A testing framework for testing lockstep-hyperproperties (merge with 1?)  
\end{itemize}
...
\leo{Example, call sites in stack safety}
...
\leo{For this the X- component is empty right?}
