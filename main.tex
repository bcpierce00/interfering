...
We assume we have an underlying step function that takes machine
states to machine states, $\MACHS \rightharpoonup \MACHS$; we also
assume the only possible partiality is because of a $\HALT$
instruction.

%The
%{\em underlying} step function, $\mathord{\stepstounderfull} \in
%\MACHS \rightharpoonup \MACHS$, embodies the standard RISC-V ISA
%semantics.  \bcp{It's probably clearer not to put RISCV in the
...
can be written to \bcp{directly or indirectly} by the network), while $\HIGHINT$ locations should be
