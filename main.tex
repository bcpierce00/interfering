...
\item For each pair of traces in the hyperproperty, all the contours
  coincide.
\item For each pair of traces in the hyperproperty, If the initial
  machine states plus contour are variants, then the observervations
...
nested lockstep noninterference is the contoured lockstep
hyperproperty where:
...
\item {\em prefix-closed:}
  ...
  For every pair of traces $\overline{(C, M)}_n$ and $\overline{(C,
    M')}_n$ in the set, the pair of truncated traces $\overline{(C,
...
\leo{..and the observations of each trace coincide, right?}
...
Formally, a policy
%
$\pol \in \exists X.\; (\CONTOURS \times \MACHS \rightarrow X)
...
\begin{enumerate*}
  ...
  \item an initializer: a function taking a contour and a machine state to an initial value of $X$,
