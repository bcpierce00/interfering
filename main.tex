...
execution traces\footnote{\bcp{I'm not certain we're using the word
    correctly.  E.g., ``A hyperproperty is a set of properties [Clarkson and Schneider 2008, 2010]''}}, are the standard way of expressing security
...
The {\em underlying} step function,
$\mathord{\stepstounderfull} \in \MACHS \rightarrow \MACHS$, embodies the
standard RISC-V ISA semantics.  \bcp{It's probably clearer not to put RISCV
  in the notation---for most of the paper, the fact that it's a RISCV really
  doesn't matter.  Moreover, I think we want to make it a partial function,
  for simplicity of definitions in this part, and if we insist on RISCV here
  this will confuse people that know it doesn't have a HALT instruction.}
...
\{\HIGH,\LOW\} \times \{\HI, \LO\}$\bcp{We need more mnemonic names for
  these (that help us keep straight when we're talking about confidentiality
  and when integrity), both in the tex and in the way they are displayed.}
is a map from machine components
to confidentiality and integrity levels. $\LOW$ marks locations whose
contents are observable from the outside (by the ``attacker,'' e.g.,
by DMA) and $\HIGH$ marks internal ``hidden'' state that cannot be
observed directly (e.g., processor registers and memory addresses for
temporary variables will usually be hidden). Similarly, $\LO$
locations correspond to low-integrity components (e.g. locations that
can be written to \bcp{directly or indirectly} by the network), while $\HI$ locations should be
protected from such flows.

\bcp{A random thought about integrity: One thing that may be helping confuse
...
A {\em trace} of length $n$, where $n$ can be finite or infinite\bcp{awkward}, is a
length-$n$ sequence $\seq{\mach}_n = \mach_0,\, \mach_1,\ldots$ of
machine states in which adjacent machine states are related by
$\stepstounderfull$---i.e., in which $\mach_{i-1} \stepstounderfull
\mach_i$ for each $i > 0$ in the sequence.

A {\em contoured hyperproperty} is a set of triples, each consisting of a
contour along with a pair of traces.
...
$\LOW$ by $\contour$---i.e., if $\contour(\component) = \LOW$, then
...
\item {\em Integrity: }
  ...
  $\contour(\component) = \HI$, $M_0(\component) = M_n(\component)$
...
\item For forward compatibility we could treat the first unmatched return as
  a halt instruction.
\item The instruction memory is marked $\LOW \times \HI$. That means
  it is observable and can't change.  \bcp{What does this assumption mean,
    precisely?  (I.e., what is ``the instruction memory''?)}
\item The memory space is split in two. One part is marked $\LOW
  \times \LO$: this is the public part (or the one that can be
  accessed by the current process e.g.). The other part is marked
  $\HIGH \times \HI$: it should be completely inaccessible to the
  current process.
...
formally?}
\item The instruction memory is again marked $\LOW \times \HI$. That
...
\item At the beginning of time\bcp{Not clear what ``beginning of time''
  ...
  right?] is marked $\LOW$), closed under an operator that takes a
  contour+trace+trace and snips out all the subtraces between a call and its
return.  (But: eek---see below.)}, the entire data memory is marked $\LOW
  \times \LO$. That makes for a very unintersting property as all
...
\item BUT, we close the hyperproperty as follows:
  ...
  \item We construct a new contour at that point: after every call,
    ...
    $\HIGH \times \HI$) until a subsequent return.
...
\apt{Again, we're conflating \VIS\ vs. \HID\ with \LOW\ vs \HIGH\; should probably be teased apart.}
...
\item Daniel's tainting paper (and some refs cited there)
      \cite{DBLP:conf/eurosp/SchoepeBPS16}
\item Nick and Andre's papers (and other micropolicy papers)
\item StkTokens: Skorstengaard, Devriese, Birkedal
\begin{itemize}
\item \url{https://arxiv.org/pdf/1811.02787.pdf}
\item \url{https://arxiv.org/pdf/1902.05283.pdf}
\item \url{http://cs.au.dk/~lask/papers/esop2018.pdf}
\item \url{https://dl.acm.org/doi/fullHtml/10.1145/3363519}
\end{itemize}
