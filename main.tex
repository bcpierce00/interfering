...
\apt{This definition may need refinement to reflect the fact that entry and exit sequences
  are multiple instructions; in the middle of these sequences the state might be neither $\ME$ nor $\NOTME$.}
...
\end{itemize}
\leo{To make this more precise we need to accurately describe what
  local state {\em is}}
...
\item $\stateobs_0$ maps every visible component $\component$ (where
  $\contour(\component) \in \{\LOWSEC \times \_\}$) to its value in
...
\item $\stateobs_{i+1}$ is defined from $\stateobs_{i}$ by (1) for
  ...
  $\contour(\component) \in \{\LOWSEC \times \_ \}$) its value
  from $\mach_{i+1}$, and (2) for each hidden component $\component$,
  where $\contour (\component) = \{\HIGHSEC \times \_\}$,
...
\apt{This definition seems to have vestiges of being based on contoured
  traces; for single contours, there is no possibility of changing from
  $\bot$ to anything else.}
...
\apt{Does this differ from lockstep equivalence?}
