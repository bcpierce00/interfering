...
\{\HIGH,\LOW\} \times \{\HI, \LO\}$ is a map from machine components
...
can be written to by the network), while $\HI$ locations should be
...
A {\em trace} of length $n$, where $n$ can be finite or infinite, is a
...
A {\em contoured hyperproperty} is a set of triples: a contour along
with a pair of traces. 
...
for example call sites in stack safety.
...
\begin{itemize}
\item Any instruction can serve as a final instruction.\leo{What about
  instructions the are halted by the monitor?}
...
\item The instruction memory is marked $\LOW \times \HI$. That means
  it is observable and can't change.
...
\item The instruction memory is again marked $\LOW \times \HI$. That
  ...
  can't change.
\item At the beginning of time, the entire data memory is marked $\LOW
