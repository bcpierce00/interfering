...
\bcp{Idea: Remove the HIGH/LOW part from contours and instead make it part
  of the $X$ parameter to both properties and policies.  (I.e., we will have
  ``X properties'' and ``X policies'' for each X.)}

A {\em contour}
$\contour \in \CONTOURS = \COMPONENTS \rightarrow \{\HIGH,\LOW\}\times
\{\OBS,\HIDDEN\}$ is a map from machine components to pairs of ``security
levels,'' where $\HIGH$ marks locations that may contain secret or
low-integrity data and $\LOW$ marks those that must only ever contain
publicly readable or high-integrity data, and ``observabilities,'' where
$\OBS$ marks locations whose contents are observable from the outside (by
the ``attacker,'' e.g., by DMA) and $\HIDDEN$ marks internal ``hidden''
...
A {\em contoured trace} is a finite or infinite sequence
$\seq{(\contour,\mach)} = (\contour_0,\mach_0),\,
(\contour_1,\mach_1),\ldots$ of (contour, machine state) pairs \bcp{such
  that the machine parts agree with the full step}.

\subsection{Observations}

\bcp{Try it the other way round: a sequence of maps from components to
  values.  Need to extend values with $\bot$ for always-hidden elements.
  Emphasize that ``observability is not observable.''}
\bcp{Also, change OBS to VIS}

A {\em component observation} is a finite or infinite sequence of words in
which adjacent elements are always distinct (``no stuttering'').  An {\em
  observation} is a map from machine components to component observations.

The observation arising from a contoured trace $\seq{(\contour,\mach)}$,
written $\obstrace(\seq{(\contour,\mach)})$, is a map taking each machine
component to the (finite or infinite) sequence of the {\em distinct} values
taken on by this component at moments when it is {\em observable}.

Formally, $\obstrace(\seq{(\contour,\mach)})$ can be constructed as
...
traces\iftext{} \bcp{Maybe prefix closed?}\fi.
...
\bcp{Not quite enough: Policies and properties needs to know where are the
  call and return sites.  So what we should do is to talk instead about ``$X$
  policies'' (i.e., make ``policy'' a type operator) and replace the initial
  $\CONTOURS$ argument to both policies and properties by an element of
  $X$.} 
...
\bcp{NI is the property where:
  \begin{itemize}
  \item In each top-level pair (each element of the hyperproperty), all the
  contours coincide.
  \item If the initial machine states plus contour are variants, then the
  observervations of the two traces coincide.
  \end{itemize}
}
...
Here we consider stack protection as an instance of generalized interference \apt{or whatever it's called}.
