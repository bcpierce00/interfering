...
using the technical framework of language-based security: a
combination of an {\em integrity property},
a {\em confidentiality property}, and a {\em well-bracketedness property}.
%
In a setting containing unstructured machine code, true
control-flow attacks are only meaningful when they bypass a caller that was
waiting for its return, so well-bracketness guarantees that callees return
only to their callers. Other control-flow attacks, like
return-oriented-programming (ROP), appear to return but keep
the callee in control while accessing the caller's data.
%
Our properties treat this scenario as the callee not returning at all,
so if the ROP attack touches the caller's data, it violates integrity
or confidentiality.

Integrity and confidentiality are also novel in that they
are ``nested'': {\em each} caller is protected from its
callees. They come in {\em stepwise} variants, in
which a caller's data is {\em never}
...
then permitted only when that activation is currently executing. On the other hand, we find that their \emph{Lazy
  Tagging and Clearing} policy
...
propose a variant of {\em Lazy Tagging and Clearing} that does enforce
confidentiality (and test that it does), albeit at some performance cost.
