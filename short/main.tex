...
allow ``lazier'' protection mechanism that permit risky reads or
writes as long as they do not affect the program's visible outcomes.
...
under hypothetical changes to the caller's stack frame.
...
Furthermore, we check incorrect
...
We have demonstrated our model's flexibility by extending it to different settings.
...
callee, and so on.)  We can also describe a simple coroutine model, with one
...
can enforce our properites.  In particular, we are adapting 
