...
under hypothetical changes to\bcp{scrambling of?} the caller's stack frame.
...
way of looking at control-flow attacks. We are not interested in\bcp{I find
  ``We are not interested in'' confusing here---the previous sentence
  promises a novel way of looking, and this opening feels irrelevant to
  that.} attacks that
merely aim to execute arbitrary code -- indeed, our model assumes
that attackers (i.e., callees) may already execute arbitrary code.
%
...
it is a violation of integrity or confidentiality.  \bcp{Actually, the whole
paragraph doesn't make very much sense to me.}
...
must always return
to their immediate caller, if they return at all. This property is
largely\bcp{``largely'' feels like a weasel word...} orthogonal to our
...
in the Coq proof assistant on top of a RISC-V ISA model\bcp{instead of
  ``spec''.  OK?}. We
...
propose a variant of {\em Lazy Tagging and Clearing} that should\bcp{``that
...
We have demonstrated\bcp{let's use the same verb tense consistently---''we
  do X'' [my preference] or ``we have done X'' or ``we did X''---all the way
through} our model's flexibility by extending it to different settings.
