...
until control returns to it. This formulation not only captures the intuition
that the callee cannot directly access the caller's state, but gives a novel
perspective on control-flow attacks such as return-oriented-programming (ROP).
...
execution\bcp{that's not very clear to me}. Our model allows the caller to
dictate the terms of a valid return -- 
...
to read the caller's state directly: it is a confidentiality
violation. Likewise, an attack that would corrupt the caller's state is an
integrity violation.

We formalize integrity and confidentiality as trace properties\bcp{hyperproperties??}, in two different
...
that Rossleer and Dehon's micro-policies correctly abort programs that
