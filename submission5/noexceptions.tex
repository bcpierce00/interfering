...
\apt{Maybe should drop the math displays from this section, and keep to words.}
Security-relevant operations consist of calls (including tailcalls), returns,
allocations and deallocations. \apt{No tailscalls for now?} Calls and tailcalls identify the address of the call's target,
and the argument registers. \apt{Either explain $\overline{sa}$ or omit it.} Allocations and deallocations are phrased in terms of a pair
...
\apt{Maybe don't need to describe $Op$ or $\stepstounder{\overline{\psi}}$ here, 
  and maybe not even combined states, although they are used at the moment.}
...
address 1000. %  the \(PC\) and \(SP\) at each step are shown for clarity.
...
\(\unsealed\). (We use \(V\llbracket\cdot\rrbracket\) to denote updates to \(V\).\apt{To avoid confusion with list brackets.} )
%
\begin{figure*}
...
registers remain \apt{right?} mapped to \(\unsealed\). All callee-save registers are always \(\sealed\)
as they were initialized\apt{???}.
...
so we define a predicate \(ret\) \apt{suggest capitalizing these predicates}
on states that should hold just after the return, namely, the program counter
should be 20 and the stack pointer should be 980. \apt{Maybe write this symbolically.}
...
To see why WBCF also cares about the stack pointer, consider an attack \apt{should this
  be shown as a fifth attack in the figure?} which
...
Stack integrity applies a similar \apt{to what?} notion to all of the memory that a function has
...
sealed objects from those that are intentionally passed to the callee. \apt{already said this}
...
addresses 980 through 999 (allocated as private\apt{??} data) and callee-saved registers such as
...
as long as it does not affect {\tt main}'s observable behavior. \apt{Maybe justify this
  point of view by appeal to standard security notions?}
...
element except those in \(\components\), and share the same policy state\apt{hopefully can remove this last}.
...
\(int\) \apt{suggest $Int$} as a predicate on states that holds if
...
a call. Let that be \((\mach,\context)\). \apt{do we need the $\context$ part??} We consider an arbitrary variant state,
