...
in the set \(\COMPONENTS ::= \WORDS + \REGS + \mathcal{P}\).
%
The set of {\em machine states} \(\MACHS\) (ranged over by \(\mach\) and \(\nach\))
defines the mapping \(\mach[\component]\) from state elements to words defined.
...
such as the program counter, are always \(\public\).
