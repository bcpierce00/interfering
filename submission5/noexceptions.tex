...
which extends the underlying machine with additional information about its
security principals and which registers and memory they require to be secure, which evolves
...
This approach is in contrast to approaches that give a safe-by-construction semantics
for various security operations and then prove secure compilation to the underlying
enforcement mechanism, in that our machine is explicitly not safe unless the enforcement
is correctly instantiated; the security semantics gives us the means to distinguish an
enforcement scheme that implements stack safety from one that does not.
...
the system, similarly, those definitions must be realistic.

Otherwise, we do not assume that the code is reasonable or corresponds to any particular
...
and are drawn from the set (\(\WORDS\)) (by default, the set of 64-bit integers).
Registers are ranged over in general by \(\reg \in \REGS\), other than the
...
is labeled by a list of security operations (order matters), defined below,
and an event.
...
{\it termination insensitive} systems \cite{}.
...
of an offset and a size. Returns don't require any additional information.
%
\begin{align*}
...
assembly code in Figure \ref{fig:f}. It might take an offset from the stack
...
proceeding upward and the stack growing down. The functions {\tt f} and {\tt publish}
are at addresses 100 and 200, respectively.

Figure \ref{fig:exec1} shows how the program counter, stack pointer, and call stack
...
maintain a two-word alignment it leaves a gap. This will have the
...
Its return target is the instruction immediately following that of the program
counter before the call, and the stack pointer target is the current stack pointer.
...
But what if {\tt f}, does not return with \(\PCname = 20\) and \(\SP = -24\),
as would be the case in the fourth example attack? Then it has violated WBCF.
This property can be phrased in terms of a single step: for every transition
labeled as a return, the program counter and stack pointer match those recorded
in the top pending activation.
...
triggering a failstop. In such a scenario we will consider a value {\it harmless}
if it could be replaced with any other value without changing the observable behavior
of the machine. That is, for a given state \(\mach\) and set of elements \(\components\),
we define a {\it variant} state that agrees with \(\mach\) on every element except those
in \(\components\). Those elements are said to be harmless in \(\mach\) if the variant
state produces an equivalent trace (see Section \ref{sec:events}.)
...
matches the state before the call. Integrity means that, at that return state,
...
not the caller's state. As we saw in our examples, the callee's behavior is defined
both in terms of events that the callee produces during the call, but also in terms of
the changes it makes to the state.
...
\(\components\)-variants. We use this to define sets of vestigial elements.
