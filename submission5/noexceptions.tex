...
Such a labeling scheme appears in our testing environment (Section [TBD]).% and is already necessary
...
\(\PCname \in \mathcal{P}\). Collectively these are {\em state elements} \(\component\)
in the set \(\COMPONENTS ::= \PCname | \WORDS | \REGS | \mathcal{P}\).
One instance of \(\mathcal{P}\) is discussed in Section \ref{sec:enforcement}.
...
A machine has a set \(\mathcal{V}\) of values such that \(\WORDS \subseteq \mathcal{V}\).
A {\em machine state} \(\mach\) is a map from components to values, written \(\mach[\component]\).
...
of the machine state. Given such a specification, we can consider
...
agree on the value of every element except those in \(\components\). This includes
public elements such as those in \(\mathcal{P}\).
...
In our example setting, the observation trace consists of the sequence of values written to {\tt out}. The example in Figure \ref{subfig:integrity} modifies the value of {\tt sensitive},
...
at the state following the callee's return, any relevant element that was \(\unsealed\) under the callee's view retains its original value.
...
Existing operations are extended to propagate \(\rho\), and the new operations work as follows:
...
\[\mathit{capped} ~ \components ~ \rho \triangleq
\bigcup_{\component \in \components} \{\component' |
\rho[\component] = (\mathit{base},\mathit{bound})
