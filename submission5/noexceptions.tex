...
\apt{As we discussed, this seems too restrictive a definition for $\mathcal{P}$.
  For example, for tagged machines it might contain a mapping from $\WORDS + \REGS$ to tags.}
...
element except those in \(\components\). \apt{Explicitly note that this excludes elements of
  of $\mathcal{P}$ (assuming that is indeed the case).} The elements of \(\components\) are \emph{irrelevant}
...
\paragraph*{Moving Forward}\apt{Suggest omit this para.}
