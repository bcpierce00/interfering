...
(2)
  ...
  for the micro-policy (see \cref{sec:enforcement}); and
(3)
  a map from instructions to lists of their operations in the security semantics.
%
\rb{((say a few more words about 3?))}
%
\rb{
...
We extend their technique with an additional
degree of statefulness:
\rb{
we keep track of the control flow behavior of
...
When complex calling conventions need to be
...
\rb{allow the generation to sometimes relax those constraints,
introducing potentially ill-formed flows:}
% allow for the possibility of,
for example, {\em not} performing such an initialization
\rb{or \emph{not} returning to the proper call site, so as}
to allow for potential attack vectors to manifest.
\rb{
This causes our programs
