...
exceptions, [tail call elimination, and concurrency.] They are modular by
...
Security mechanisms designed to prevent a certain class of attacks can be brittle---successfully
...
properties that can be proven or tested. Such properties become the specification against which
enforcement can be validated; even enforcement mechanisms that do not fulfill the property
benefit from the ability to articulate {\it why}.
...
return from a callee to a distant ancestor is perfectly reasonable. Subtle tweaks to the definition
accompany any addition to the model.
...
not all enforcement mechanisms attempt to implement all of them. Some focus exclusively on
control flow, others protect the confidentiality of the caller but not the callee, etc.
...
the following table. Those that we explore in detail in Section \ref{sec:examples}
are shown in white: calls, returns, and the allocation and deallocation of local memory.
In the full formalism (Appendix \ref{app:full}) and the machine that we test,
...
based on pointer provenance in Appendix \ref{app:ptr}, represented by the dark grey
