...
\begin{figure}
  ...
  \begin{subfigure}{\columnwidth}
    ...
    \(\textnormal{\large \dots} \pubbox[30px] \!\! \stackrel{\textsc{\large sp}}{\textbox[\tt res]}
    \stackrel{\textsc{\large 4(sp)}}{\textbox[\tt sens]}
    \stackrel{\textsc{\large 8(sp)}}{\textbox[\tt sec]}
    \stackrel{\textsc{\large 12(sp)} \hfill}{\textbox[\(\mathtt{ra_1}\)]\textbox[\(\mathtt{ra_2}\)]}
...
{\tt secret}.
In \cref{subfig:WBCF2}, the attacker returns with \(\SP' = 998\) instead of the
...
Using this mechanism, an argument passed on the stack via its \(\SP\)-relative offset
uses the operation \(\mathbf{call} ~ \dots ~ (\SP, \mathit{off}, \mathit{sz})\).
C++-style reference passing that places the reference in \(\reg\) uses
\(\mathbf{call} ~ \dots ~ (\reg, 0, \mathit{sz})\).
In the second case, the reference can be passed further down the stack, but otherwise
only the immediate callee gains access.
...
object that recieves no protection. We extend the \(\mathbf{alloc}\) operation
