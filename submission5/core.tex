...
of this section we introduce the underlying \bcp{semantic?} framework
in more detail.
...
\paragraph*{Security Semantics and Property Structure}\bcp{Delete
...
single underlying machine instruction\bcp{That sentence is opaque.}.
Since the same machine instruction may be used by compilers for
...
an optional external\bcp{observable?} event, and \(\bar{\psi}\) is a
list of security-relevant operations\bcp{that bit is not completely
  clear---e.g., why a list?  because the same
machine step can do several distinct security-revevant things}.
%
This is then lifted into a transition between pairs of machine states
and contexts
by applying a set of operation-specific rules\bcp{why is it a set of
  rules rather than a single transition function?} that, given a
state and associated state, yield a new context corresponding to
the new state calculated by the ordinary transition relation on states.
%
...
\(\downarrow \overline{\psi}\), between steps. We write \(\emplist\)
for empty lists.  \bcp{Is it ever anything other than empty or a
  singleton?}
...
\noindent
{\tt main}'s attempt to read {\tt sensitive} will\bcp{may?} instead
