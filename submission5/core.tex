...
Recall\apt{make sure this was said in previous section!} that our security semantics attach a security context to the machine state,
...
memory to \(\public\). The sole used argument register, {\tt a0}, is mapped to \(public\) \apt{why not \(unsealed\)?};
...
\begin{figure*}
  \begin{tabular}{|r|r||l|r}
    ...
    \multirow{3}{*}{\(\underbrace{\dots \freebox \freebox \freebox \freebox \freebox
      ...
      ~ \stackrel{\mathtt{a0}}{\pubbox} ~ \stackrel{\mathtt{a1}}{\freebox}
...
%      ~ \stackrel{\mathtt{s0}}{\sealbox}
    ...
    \multirow{3}{*}{\(\underbrace{\dots \freebox \freebox \freebox \freebox \freebox}_\unsealed
      ...
      \! \underbrace{\pubbox \pubbox \pubbox \dots}_\public
      ~ \stackrel{\mathtt{a0}}{\freebox} ~ \stackrel{\mathtt{a1}}{\freebox}
%      ~ \stackrel{\mathtt{s0}}{\sealbox}
      \)}
    \\
    \cline{1-3}
...
mapped to \(\public\), while any non-argument, caller-saved
...
are the same in \(\mach\) and \(\nach\). \apt{This is a confusing formulation! Introduce ``corrupted'' here?}
We then define return-time confidentiality as holding if \(\confProp\) holds on the matching return states
from \(\mach\) and \(\nach\), respectively.
...
\apt{We may need a new section that describes how a tag-based system would enforce the properties
  described so far.}
...
%    & \(\textnormal{ when } (\mach,(V,\sigma)) \textnormal{ is (tail)called}\) \\
   ...
   \rowcolor{black!20}
    ...
    \mach' \parallel \bar{\Diamond}(\mach,\nach,\mach',\nach')\) & \\
  \end{tabular}
  \caption{Properties. \apt{Something wrong with last one. $\cintProp$ vs. $\confProp$? }}
  \label{tab:props}
\end{table*}
