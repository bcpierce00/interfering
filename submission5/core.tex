...
of features such as calls and returns, but is not itself a feature.
...
that maps each {\em state element} of the machine (memory location or register)
...
security properties---integrity, confidentiality, and well-bracketed control flow---
as predicates that must hold on each call. These predicates draw on the idea of
...
delaying checks until a corrupted value would actually impact visible behavior.
...
To use QuickChick, we develope random test-case generators that produce
...
increase the probability of initializing any stack-allocated variables.
However, periodically, the generator
...
property. A typical hyperproperty testing scheme might do so by
...
For performance, we instead maintain a single ``post-call taint''
