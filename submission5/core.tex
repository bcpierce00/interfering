...
\apt{Can we connect stack safety more explicitly to a broader idea of ``subroutine safety''
  as discussed in the intro? Note that this also includes registers.}
...
and are we successful?\apt{For this audience, the need for a \emph{formal}
  characterization doesn't need to be belabored.}

In this paper we focus \apt{Is this still accurate?} on one such non-intuitive enforcement mechanism,
...
\apt{Remainder of this section should be moved to AFTER the rest of the formalization.}

\subsection{Sharing Stack Memory}
In our examples so far, we have presented a vision of stack safety in which
...
\apt{Duplicates title of last section.}
