...
(\(\clrc\)), should catch writes to sealed objects
if they are later read (\(\clri\)), and should prevent reads
from deallocated objects (\(\clec\).)
They also note that Lazy Clearing prevents uninitialized reads,
which corresponds roughly to \(\clei\).
