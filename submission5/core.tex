...
lazy enforcement is incompatible with existing models of stack safety,
because an attacker can modify another function's data. But the owner of
the data will never see the changes, so it is secure in the sense that the attacker is
unsuccessful at changing program behavior (except, perhaps, by causing a fault.) We aim to elaborate
...
normal ``eager'' micro-policies, and should support other enforcement mechanisms. In fact, our
observational properties are even weaker than necessary to support Roessler and Dehon's micro-policy.
...
In \cref{sec:machine}), we cover additional operations add parameters to the examples', to support
...
We have five different properties to introduce:

\begin{itemize}
\item Well=bracketed Control Flow (\(WBCF\))
\item Caller Integrity (\(CLRI\))
\item Caller Confidentiality (\(CLRC\))
\item Callee Confidentiality (\(CLEC\))
\item Callee Integrity (\(CLEI\))
\end{itemize}

The properties form a rough hierarchy; although they are technically separate and need
not coexist in any particular combination, some are likely to take priority in terms of their
...
Many also cover some degree of (caller) integrity. [TODO: more hierarchy].
...
develops over the first few steps of the program, beginning with
the start of {\tt main} (\(\PCname = 0\)) and with the stack pointer at address 1000.
...
Security classes are ranged over by \(C\).
For some security class \(C\), we write \(C(V)\)
to denote the set of elements \(\component\) such that \(V ~ \component = C\).
...
[TODO: back-propagate abbreviations used here]

Finally, with the preliminaries out of the way, we can define our core properties. We write
``when \(s\) is called'' as a shorthand for quantifying over all \(s\) reachable in any execution
such that \(s \stepstounder{\overline{\psi}} s'\) and
...
\begin{table*}
  \begin{align*}
    ...
    %
    \mathit{ClrI} \triangleq & (|\sigma| \uparrow \intProp) ~ (\mach,(V,\sigma))
    && \textnormal{ where } \intProp ~ \mach' \textnormal{ iff }
    \mach' \parallel \sealed(V) \cap \Delta(\mach,\mach')
    && \textnormal{ when } (\mach,(V,\sigma)) \textnormal{ is called} \\
    %
    \mathit{ClrC} \triangleq & \forall \nach \textnormal{ s.t. } \mach \approx_{\sealed(V)} \nach, &&
    && \textnormal{ when } (\mach,(V,\sigma)) \textnormal{ is (tail)called} \\
    & |\sigma| \hookrightarrow (\mach,(V,\sigma)) \simeq |\sigma| \hookrightarrow (\nach,(V,\sigma)) \\
    & \textnormal{ and } (\mach,(V,\sigma)) ~ (|\sigma| \Uparrow \confProp) ~ (\nach,(V,\sigma))
    && \textnormal{ where } (\mach' ~ \confProp ~ \nach') \textnormal{ iff }
    \mach' \parallel \bar{\Diamond}(\mach,\nach,\mach',\nach') \\
    %
    \mathit{CleC} \triangleq & (|\sigma| \uparrow \cconfProp) ~ (\mach,(V,\sigma))
    && \textnormal{ where } \cconfProp ~ \mach' \textnormal{ iff }
    \mach' \parallel \unsealed(V) \cap \Delta(\mach,\mach')
    && \textnormal{ when } (\mach,(V,\sigma)) \textnormal{ is (tail)called} \\
    %
    \mathit{CleI} \triangleq & \forall \nach \textnormal{ s.t. } \mach \approx_{\unsealed(V)} \nach, &&
    && \textnormal{ when } (\mach,(V,\sigma)) \textnormal{ is (tail)called} \\
    & |\sigma| \hookrightarrow (\mach,(V,\sigma)) \simeq |\sigma| \hookrightarrow (\nach,(V,\sigma)) \\
    & \textnormal{ and } (\mach,(V,\sigma)) ~ (|\sigma| \Uparrow \confProp) ~ (\nach,(V,\sigma))
