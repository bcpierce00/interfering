...
implement functions, aided by an ABI\bcp{will all CSF readers know
  what an ABI is?} that defines how registers are shared
between activations.
...
These are by no means exhaustive\bcp{That comment belongs in the
  future work
  section, not here}. We apply these first to a simple system
...
that use variants\bcp{undefined},
...
any additional enforcement information (such as a tag).\bcp{Why?}
...
\proposeadd{single?} underlying machine instructions,
...
A call pushes a new activation view\bcp{it was just called ``view'' above} onto the context stack and changes the class of the
...
\proposecut{Our property framework has many parameters to support broad applicability.}
...
\apt{Suggest moving threat model to here.}\bcp{Fine.}
