...
\proposecut{While our properties are motivated by the desire to specify \(LTC\) precisely,
they are stated abstractly in hopes of applying
to other enforcement mechanisms.}\apt{I think we should try to keep something
  along these lines in the intro, if not this particular sentence.}
...
mechanisms.
\item
  %We instantiate\bcp{extend??} these definitions in a realistic
...
are kept secret, it suffices to vary their contents them and check that the observable
output does not change.

%Non-interference is a way of expressing
...
(hyper-properties).\bcp{This is the first we've mentioned
  hyperproperties.  CSF readers may be surprised.}
\sna{Surprised that we have hyperproperties,
  or that we made it this far without mentioning them?}
\bcp{The latter!}

Security-relevant operations typically correspond to
single underlying machine instructions,
...
instructions might also be used for other things. Thus we identify
security-relevant operations by labeling selected instructions. 
%% An instantiation of
%% an operation
%% might require some external mechanism for distinguishing which instructions are
%% security-relevant.\bcp{Huh?  What's an instantiation of an operation?} The most obvious technique,
...
%\bcp{I was confused by the table because I looked at
% the figure before I read that sentence!}
%Those that we explore in detail in \cref{sec:example,sec:formal}
...
\bcp{As a reader, I'm feeling a bit overwhelmed at this point.
  Everything feels fluid and flexible---''It could be like this, but
  it could also be another way if you want...''  I know we're past the
  point of doing more than tweaking the way we talk about things, but
  if there are any tweaks that reduce the sense of flexibility, let's
  do them!}

\sna{Agreed. The point of the operations was originally that we shouldn't
  have to think about all of these things when presenting the formalism,
  not that we should open with all the different ways we can think about them.}

\apt{Hopefully better now.}
...
\bcp{cut this?}In \cref{sec:example} we give such an instantiation, and
...
them\leo{where do we mention reasoning about them?}, and to leverage the power of the QuickChick % property-based
...
from trivial typos to ones that require a %n intriguingly
complex setup
...
%
%The micro-policies of
Roessler and
DeHon \cite{DBLP:conf/sp/RoesslerD18} are the most flexible in supporting a
range of safeguards including lazy enforcement, and suitable to formulate
and validate strong security guarantees.
...
Most of those define experimental types of hardware capabilities to achieve
...
properties.
%
Originally, they describe ``local state
encapsulation''~\cite{SkorstengaardSTKJFP} in terms of integrity (but it is
equivalent to % the combination of
\(\clri\) {\em and} \(\clrc\)). Their latest
...
\sna{See commented out tex for content that could be massaged to fit in here,
  depending on whether we're interested in delving deeper.}\bcp{If
  there's space, some of this could be resuscitated.}
\apt{It would be very good to do this, because reviewers will want a careful comparison with this work,
  but I don't feel quite confident enough to do it (especially after my unfortunate attempt last time)!}
...
\proposecut{HardBound \cite{DeviettiBMZ08} is another hardware extension related to
...
features and different security requirements. Another 
...
\proposecut{Having formalized our definitions in Coq gives us a head
  start in this effort.} \bcp{Or just just the whole paragraph.}
...
\apt{Sbould admit up front that this extension requires a fundamental change in the
  security context.}
...
Note that we have a degree of monotonicity \apt{??} here. Once an object is sealed (because its
