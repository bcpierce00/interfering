...
high-level language concept, and mechanisms for protecting it
typically lack precise specifications,
relying instead on informal descriptions and examples of bad
behaviors that they prevent. A more rigorous specification is especially desirable
for unusual enforcement mechanisms\bcp{why especially for this?}
like the ``lazy'' stack safety
...
and\bcp{two?} confidentiality properties\bcp{plural?} (``the caller's
and callee's behavior are insensitive to one another's private
state,''), both compatible with lazy tagging\bcp{undefined term; also,
the fact that we've mentioned tagged enforcement twice already makes
it really feel like we are proposing an abstract theory of exactly one
thing}, plus a control-flow property.
...
a more generic characterization to the reader's intuition.  But these
...
attacks in a given class---or, conversely, whether it is overly conservative and disallows
...
the literature. \bcp{can we give a very quick preview here of the comparison?}
...
during the callee's execution, the confidentiality of the callee's local state
afterward, and the control flow protection of {\em well-bracketed control
  flow (WBCF)}~\cite{SkorstengaardSTKJFP}. We also extend WBCF to a setting with exceptions,
in which there may be multiple points at which a callee can ``return.''

This approach has several benefits: first, not all state-safety enforcement mechanisms
aim to protect all aspects of stack safety. Most protect control flow, but relatively
few care about the confidentiality of the callee after return. Second, our formalism
factors stack safety into three questions: (1) ``when does this machine call, return, etc.?''
(2) ``during any given call, which elements of the state are in need of protection?'' and
(3) ``when an element needs protection, how can we tell that it is protected?''
Different machines, including those with different enforcement mechanisms, may answer question
(1) differently, without needing new definitions for (2) or (3). In order to adjust our notion
of permissible behavior to correspond to features such as pass-by-reference behavior, which
alter its security requirements (typically by relaxing them), we need only add nuance to (2).
Regardless of the assumptions that we make for either (1) or (2), the security definitions that
answer (3) need not change.
...
immediate callees (which, in turn, are protected from their immediate
callees, etc.).
...
part of a run, requiring that the callee’s behavior be
invariant under hypothetical scrambling of the caller’s stack
but allowing the caller to access its own data upon return.

Our threat model is very strong: it allows the attacker
to execute arbitrary code, including attempting to smash
the stack to disrupt the program’s control flow. Our stack safety
properties promise that, even in the presence of such attackers, confidentiality
and integrity still apply.
%

For confidentiality and integrity, our properties\bcp{by this point,
  the reader may be getting confused about why we are talking about
  multiple properties, rather than just one (with multiple clauses)} are {\em observational},
...
system. This permissiveness enables our properties to handle lazy
enforcement mechanisms.  \bcp{do we have any in mind aside from Nick's
tagging-based ones?}
...
\item We formalize permissive stack confidentiality and integrity properties.
  ...
  data causes a visible change in system behavior %(\cref{sec:lazy}).
\item We apply\bcp{instantiate?} these definitions to a complex\bcp{is
  its complexity a selling point?} system
  with features such as argument passing
  on the stack, callee-saves registers, exceptions, and a simple
  coroutine system% (\cref{sec:ext}).
  \bcp{Are these features really independent of the generic model?  If
  so, that's a nice point that should be brought out clearly.}
\item We use property-based random testing to validate the relationship between
  ...
  \bcp{Micro-policies have not been explained, and we haven't
    motivated why this would be an important or useful thing to do.}
\end{itemize}
%\Cref{sec:relwork,sec:future} discuss related and future work.
\bcp{Why did the section references get taken away from all that?  And
why delete the last sentence?}
...
\bcp{Make clear that there are no proofs!}
...
\section{Machine Model and Security Semantics}
  ...
  (if any).  Our machine model is meant to be a faithful description of the\bcp{??} hardware,
...
in which our security-relevant operations are restricted to calls,
...
% \begin{itemize}
% \item
function calls and returns, with caller- and callee-saved registers,
% \item
allocation of strictly private and strictly public regions on the stack,
% \item
arguments spilled onto the stack and/or passed by reference,
% \item
exceptions,
and
% \item
...
functions which have received a valid pointer to them.  \bcp{???}
...
\sna{There is no concurrency in this document. Attempting to build it in from the beginning
  ...
  from a technical standpoint, subject to the one-thread-per-processor constraint.}\bcp{Appendix?}
...
A {\em machine state} \(\mach\) is a map from state elements to a set \(\mathcal{V}\) of
...
{\em Events} are drawn from a set \(\OBSS\)\bcp{lowercase?} and ranged over by \(\obs\), with a
...
produce output\bcp{why?}, an assumption that gives our properties the flavor of
{\it termination insensitivity}.

\subsection{Security-Relevant Operations, Principals, and State}

Security-relevant operations consist of calls, returns, and allocations.
Calls identify the address of the call's target,
plus the argument registers. Allocations and deallocations are phrased in terms of a pair
...
on a RISC-V machine with a PIPE tagging system\bcp{undefined}. Here
...
\(\mathcal{T} ::= \mathit{gen} \mathrel{|} \mathit{bseq} ~ n ~ m \mathrel{|} \mathit{prot}\), named for general
...
To illustrate why this matters, let's consider two integrity
micro-policies: ``Depth Isolation'' and
...
of values written to {\tt out}.\bcp{Is this the first place we've
  discussed what the observations actually are?  Is this the only
  thing they could be, or does the model support other options?}
...
both limitations.\bcp{Then why are we not comparing to them instead??}
...
\subsection{Facts About Calls and Returns}
\label{sec:facts}
...
parts happen when they interact through memory. \bcp{This seems
  infeasibly hard to address at this point.}
