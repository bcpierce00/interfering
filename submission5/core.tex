...
\sna{Clarify that we do not mean that the notion of a stack is absent, but
  rather that it is precisely the expectations referenced by reviewer 2 that
  we are trying to codify. Priority 2.}
...
\sna{Clarify, per reviewer 2. Priority 2. Also consider connections to robust
declassification. Priority 4.}
...
\(\Delta(\mach,\nach) \subseteq \components\). \sna{define \(\sim\). Priority 2.}
...
Their connections to Roessler and DeHon's work is discussed below.
...
\paragraph*{Applicability to R\&D}

In Roessler and DeHon, Lazy Tagging and Lazy Clearing are both optimizations that
...
Roessler and DeHon differentiate between memory safety policies (without lazy optimization)
and {\em data-flow integrity} policies (with lazy opimization.) Our properties
are phrased in terms of data flow, which corresponds to the optimized policies,
and which we argue is also sufficient to test non-optimized Depth Isolation.
Explicit formal properties are out of their scope, but they do list the
behaviors that they expect their data-flow integrity policies to prevent.
Data-flow integrity policies should prevent reads from sealed objects
(our caller confidentiality), should catch writes to sealed objects
if they are later read (our caller integrity), and should prevent reads
from deallocated objects (our callee confidentiality.)
They also note that Lazy Clearing prevents uninitialized reads,
which corresponds to our callee integrity.

They note a flaw in Data-flow Integrity: because function activations
...
that was previously written by an activation at the same depth, it will
...
Roessler and DeHon propose addressing the dangling-pointer issue by
tracking both the depth of the current activation and the identity
of the active function. This would not eliminate all instances, but it
...
We propose instead tracking every activation uniquely, which in our tests eliminates
the issue entirely.

\paragraph*{Protecting Registers}

Roessler and DeHon do not need to protect registers, since the compiler is in their
...
\sna{Discuss false positives, false negatives, and the degree to which
  testing can increase our confidence. Priority 2.}

\subsection{Test Generation}
\sna{Restore Leo's old test gen discussion, then ping Leo if it seems that more
  is needed. Priority 2.}
