...
The latest stack-safety overlay semantics \cite{Georges22:TempsDesCerises} capture
both control-flow and data protection (confidentiality and integrity).
But, this approach is not suitable for {\em lazy} enforcement mechanisms like those
...
an architecture-specific construct. Its desired behavior varies based on ISA, calling convention,
and the features of whichever programming language originated the code that must be protected.
Our approach centers around a tagged architecture, like PIPE \cite{???} or STAR \cite{???},
particularly using a lazy micro-policy. However, our design is factored into three pieces:
(1) the definition of {\em security relevant operations} in a given machine,
(2) the definition of a {\em security context} which tracks the security needs of different
elements of the state over the course of execution, and
(3) the definition, for a given security context, of how we distinguish secure behavior
from insecure. This separation of concerns allows (3) to be relatively general, while
architecture-specifics are isolated in (1) and some special features require extending
(2).
...
\sna{Version that is less tied to lazy enforcement. It's still rather tied, actually,
