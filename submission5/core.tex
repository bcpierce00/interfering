...
of features such as calls and return, but is not itself a feature.
\footnote{Contrast Azevedo de
...
and well-bracketed control flow---in terms of the security classes of {\em state elements}
(memory addresses and registers). The properties are predicates use variants, assertions
about future return states, and comparison of event traces to capture
...
if we varied the assembly code for the example so that {\tt sensitive} was stored
...
\sna{Consider connections to robust declassification. Priority 4.}
...
\judgmentthree{\((\mach,(V,\sigma)) \stepstounder{\obs} (\mach',\context')\)}
              {\(|\sigma| \geq d\)}
...
object they first pointed to, and propagates the annotation
...
this enhancement is more speculative than the others, and we do not test it.
...
issue entirely, as it does in our tests.
...
micro-policy. We choose Coq for our implementation
...
%
QuickChick is a randomized property-based testing framework. We use it to generate
random programs, then execute them and test that they fulfill our criteria.

Such testing is sound---it will not produce false positives---but necessarily
necessarily incomplete. We might test a flawed policy but fail to find a
...
to find a dangerous program but have it pass the test due to variant selection.
We can increased our confidence in our test coverage by {\em mutation testing},
in which we intentionally inject flaws into our policies and demonstrate that our testing
can find them.
...
low-level program. They must also produce the meta-information about
how instructions in that program map to security-relevant operations,
information that would normally be provided by the compiler.
...
We extend Hrit\c{t}cu et al.'s technique with additional statefulness
to avoid early failstops. For example, immediately
after a call we initialize any stack-allocated variables
with an increased probability. However, periodically, the generator
will relax those constraints and generate potentially ill-formed
code, such {\em not} performing such an initialization, writing outside
...
schemes to handle the nested nature of confidentiality. In normal
noninterference testing, we would generate a pair of initial
variant states, execute them to completion, and compare the results.
Instead, for our setup to test the confidentiality of every call,
it would need to create a variant state at each call point, execute
that until return, then generate a post-call variant based on any tainted
values that would execute alongside the ``primary'' execution until
completion. Tracking a number of variant executions that is linear
in the total number of calls is rather inefficient! For performance,
we instead track a single ``post-call taint'' execution. This machine
executes in parallel with the original. Everytime a function returns
with elements containing tainted data, those elements are varied in
the tainted execution. So, at any given time, we need only simulate
...
makes testing longer runs substantially faster, at the cost of making it
...
%
These works define stack safety as overlay semantics and related stack
...
\sna{Look into wasm compilation per reviewer 1. Priority 4.}
...
and by Deutsche Forschungsgemeinschaft (DFG, German Research Foundation) as part of the Excellence Strategy of the German Federal and State Governments, EXC 2092 CASA -- 390781972.
