...
\apt{Still working from here down}
...
\cref{tab:psi}.\bcp{I was confused by the table because I looked at
  the figure before I read that sentence!} Those that we explore in detail in \cref{sec:example,sec:formal}
...
based on pointer provenance in \cref{app:ptr}\bcp{first
  mention of this appendix; if it's important, mention it in the
  outline in section I.  Otherwise drop it.}, represented by the dark gray
...
registers\bcp{what is a globally-relevant
register?}---are always labeled \(\public\). We place security requirements on some
...
When the active function\bcp{are functions active, or are their
  arguments active?} allocates memory, that memory will also be \(\object\).
...
when constructing variants (see below); the rest of the value may contain
information relevant to a hardware enforcement mechanism (such as a tag), which
...
In a tag-based enforcement scheme, values are compatible if their tags are identical.
...
\sna{See commented out tex for content that could be massaged to fit in here,
  ...
  there's space, some of this could be resuscitated in the related
  work section.  But removing it from here is a good call.}
...
features and different security requirements. Another \proposecut{important dimension}
is applicability to multiple enforcement mechanisms. \apt{Could we mention that
  in intro as well?}  \proposecut{There is much future work in each of these dimensions.}
