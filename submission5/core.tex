...
which is broadly consistent across architectures and therefore amenable to a very general approach.)
...
exclusively on control flow, others protect the confidentiality of the caller but not the
callee, etc.
...
and of compatibility with existing codebases. In the motivating case for this work,
...
Roessler and DeHon find that the obvious way to enforce stack safety in this setting
...
when a function writes to memory. This means that such writes cannot be protected. Instead
the micro-policy enforces that memory be read by the same fuction that wrote it. This
lazy enforcement is incompatible with existing models of stack safety, particularly the
integrity side, because an attacker can modify another function's data. But the owner of
the data will never see the changes, so it may still be secure in some sense. We aim to elaborate
that sense, and to test whether the micro-policy fulfills it.
...
\item Our formalization of well-bracketed control flow in the presence of tail-call
  elimination is a slight novelty over existing models.
...
informally. Then in \cref{sec:machine} we will introduce the formal machine model and give
its {\em security semantics}, described below. In \cref{sec:props} we give abbreviated
formal statements of the properties, while the full formalism is found in \cref{app:props}.
\Cref{sec:ext} gives the security semantics for the full, tested machine, and
\cref{sec:testing} describes the testing framework used to test it.
...
\cref{tab:psi}. Those that we explore in detail in \cref{sec:examples}
are shown in white: calls, returns, and the allocation and deallocation of local memory.
In the full formalism (\cref{app:full}) and the machine that we test,
we cover additional operations add parameters to our existing ones, to support
various forms of memory sharing and tail-call elimination. These are shown in light gray.
...
\begin{table}
  \begin{tabular}{| l | l |}
    ...
    \hline
    \example \(\mathbf{call}\) & Target address \\
    \example & Argument registers \\
...
\paragraph*{Events, Steps, and Traces}
...
Events are ranged over by \(\obs\), and include the special silent event
\(\tau\). Traces are non-empty, potentially infinite (coinductive) series of
events, and traces are {\em similar} \(\obsT_1 \simeq \obsT_2\) if they are
equal up to deletion of \(\tau\) events. This means that an infinite silent trace is
similar to any trace, consistent with termination insensitivity.

A machine step function
\(\mach \xrightarrow{\overline{\psi},\obs} \mach' \in \MACHS \rightarrow
\MACHS \times \mathit{list} ~ \Psi \times \OBSS\)
is labeled by an ordered list of security-relevant operations and an event.
We lift it into the security semantics by adding a security context to the
state, giving us a combined step function
\(\mach,\context \stepstounder{\overline{\psi},\obs} \mach',\context' \in
\MACHS \times \CONTEXTS \rightarrow \MACHS \times \CONTEXT \times \mathit{list} ~ \Psi \times \OBSS\).
We will define \(\stepstounder{}\) informally in the next section, and formally
in \cref{sec:props}.
...
By sheer luck, the callee has managed not to impact the observable execution beyond that one write.
Luck, or enforcement: Lazy Tagging and Clearing, from Roessler and Dehon, works by
failstopping any time a caller accesses callee-tainted data, so that out-of-frame
writes should never impact execution.
...
Recall from \cref{sec:ideas} that a machine is a map from state elements
to values, with a step function \(\mach \xrightarrow{\bar{\psi},\obs} \mach'\).
The step function is total---while some enforcement mechanisms may cause a machine
to failstop, we model this as the machine silently diverging by remaining in the
same state.
...
\(\mathbf{alloc}\), \(\mathbf{call}\), and \(\mathbf{ret}\). Their precise behavior
is found in \cref{fig:basicops}.

\begin{figure}
\judgmentbr[Alloc]
           {\(b = \mach[\SP] + \mathit{off}\)}
           {\(V' = V \llbracket \addr \mapsto \object |
...
all functions until deallocated.
...
\begin{figure*}
  \begin{subfigure}{0.4\textwidth}
    ...
    \caption{Allocating Public Regions}
  ...
  \begin{subfigure}{0.6\textwidth}
    ...
    \judgmentbr[]
               ...
               {\(Op ~ \mach ~ (\mathbf{tailcall} ~ \addr_{target} ~ \overline{\reg_{args}} ~ \overline{sa})
                 ~ (V,\sigma) = (V',\sigma)\)}
    \caption{Argument Passing on Stack}
    \label{sfig:stkargs}
  \end{subfigure}
...
\subsection{Properties at Return}
\label{sec:facts}
