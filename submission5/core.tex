...
this abstraction in memory. \an{registers?} From a security perspective, attacks on the stack are really attacks
...
Indeed, the call stack is a ancient~\cite{one06phrack} and perennial~\cite{mitre-cwe,DBLP:conf/raid/VeendCB12,
  ...
  target for low-level attacks. \apt{characterize the variety of attacks?}
%
...
Indeed, the Cerise work of Skorstengaard et al. \cite{SkorstengaardSTKJFP}\apt{that's not Cerise} and
Georges et al. \cite{Georges22:TempsDesCerises} is so far the sole line of work
to offer (various)\bcp{Maybe we can omit that word here and spell it
  out in the related work.} stack safety formalizations.
...
%We argue that a formal characterization of \(LTC\)'s protections
...
The paper is structured as follows. In \cref{sec:example}, we walk through a function
call in a simple example machine and discuss informally how each of our properties applies to it.
In the process we discuss some motivation for the properties, from a security
...
\apt{Agree with AN that this should come first instead of last.}
