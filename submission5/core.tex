...
``the heap,'' the hardware stack does not correspond to a single
...
This presentation is motivated by a particular class of enforcement
...
The hardware stack interacts with a large number of language features,
some of which are typically excluded from discussions of stack
safety. Our properties go further than previous proposals in
supporting caller- and callee-saved registers, arguments passed on the
stack, \ifexceptions tail-call elimination, and exceptions.  \else and
tail-call elimination. \fi They are modular by design, in order to be
further extensible to other features.
...
and DeHon's original lazy policy.  A fixed version of their policy
does pass our tests.
...
We assume a conventional hardware ISA, such as x86-64, ARM, or RISC-V, with a program counter and stack pointer,
and a standard ABI with a contiguous stack that holds interleaved data and control information.
The machine might be enhanced with additional monitoring or other security mechanisms,
such as hardware tags or capabilities. We do not , but....
