...
that call one another to define larger computations in a modular way.
%
At a low level, each function activation manages its own data---local
variables, spilled temporaries, etc.---as well as information about the \emph{caller} to
...
offering formal guarantees: they may impede attackers, but do not provide
...
typically formal ones.  To our knowledge, the sole
...
Mechanisms besides this line of work should also be amenable to strong
...
universal protection, but not yet tied to a particular formal guarantee.
...
and the callee's behavior during (and after) the callee's execution, plus the
...
they are stated abstractly in the hope that they can also be applied
...
definitions of most source languages. It is implied by the semantics
of features such as calls and returns, but is not itself a feature.\footnote{
Contrast Azevedo de
...
%
But neither can stack safety be an entirely satisfying low-level property;
...
%These are by no means exhaustive\bcp{That comment belongs in the
%  future work
%  section, not here}.
...
We show that our properties are useful for distinguishing between correct and
incorrect enforcement using QuickChick~\cite{Denes:VSL2014,Pierce:SF4}, a property-based
...
a simple example machine and discuss informally how each of our properties
applies to it. In the process we motivate the properties
...
should give a URL or other pointer.} contains formal definitions (in Coq) of our
...
of rules that describe how the machine code implements the function abstractoin
using the stack and registers.

Given the security classes of the machine's state elements, we define high-level
security properties---integrity, confidentiality, and well-bracketed control flow---as
predicates that must hold on each call. These predicates draw on the idea of
\emph{variant} states from the theory of non-interference, and a notion of
...
secret, it suffices to compare the execution of states which vary at those locations
and check that their traces of observable events are the same. This structure
allows us to talk about the eventual implact of leaks or memory corruption without
reference to internal implementation details, and to support lazy enforcement by
...
an (optional) external event, and \(\bar{\psi}\) is a list of security-relevant operations.
%
...
architecture might place additional constraints, particularly on the behavior of
...
\begin{figure}
  \newcommand{\figonebox}[1][]{\genbox{30pt}{lightgray}{#1}}
  ...
  \caption{Example: C and assembly code for {\tt main}, and layout of its stack frame; the stack grows to the left.\apt{In this and later figures, can we lighten the blue backgrounds?}}
  \label{fig:main}
\end{figure}
...
\footnote{We pad some of the variants with {\tt nop}s just so that all the
...
\(\downarrow \overline{\psi}\) between steps. We write \(\emplist\) for empty lists.
...
if, in the example, we varied the assembly code for {\tt main} so that {\tt sensitive} were stored
...
\sna{Might need to make this concept a touch more explicit in \cref{sec:props}, or else
promise a little less here.}
}
...
\begin{figure}
  ...
  \caption{Integrity Violation\apt{Unclear how these states are related. Extend caption?  And what do the colors means?}}
  \label{fig:variant}
\end{figure}
...
assuming that {\tt sec} is 5. \apt{But the figure seems to show several states. Unclear!}
Similar to \(\wbcf\), we define
...
\apt{Rest of this paragraph is still not very clear, especially in (non-?)relation to the figure.}
Consider a variant state in which {\tt sensitive} has any other value, % arbitrarily choosing
...
Consider the state after step 5,
% APT: trying to remove mysterious state notation:
% Consider the state \(\mach\) after step 5, whose context is \((V_2,\sigma)\),
5with the attacker code from \cref{subfig:direct}. We take a variant state over the set of elements that are
...
4 \apt{3???} (its value in the variant) respectively. This is a violation of
...
\end{figure}
%
In this case, {\tt a0} has changed during the call, {\em and}
the return states do not agree on its value. We can therefore deduce that
it carries some information derived from the original varied elements. By
contrast, the sealed stack frame has not changed during the call, so the
fact that it varies between the return states represents that the initial
states disagreed, not a leak.
\apt{Last sentence still quite mysterious.}
...
\sna{Consider connections to robust declassification. Low priority.}
...
to denote the set of elements \(\component\) such that \(V ~ \component = l\).\apt{Did you ever use this?
  I've edited some places to do do.}
...
\begin{figure}
  ...
  \judgmentbr{\(\components = \mathit{range} ~ \SP ~ \mathit{off} ~ \mathit{sz} ~ \mach \cap \unsealed(V)\)}
    ...
    \judgmentbr{\(V' = V \llbracket \reg \mapsto \unsealed | \reg \in \mathit{CLR} \rrbracket
              ...
              {\(V'' = V' \llbracket \addr \mapsto \sealed | \addr \in \object(V')\rrbracket\)}
               {\(Op ~ \mach ~ (\mathbf{call} ~ \addr_{target} ~ \overline{\reg_{args}})
                 ~ (V,\sigma) = (V'',V::\sigma)\)}
    %
    \judgment{}
             {\(Op ~ \mach ~ \mathbf{return} ~ (\_, (V,\sigma')) = (V, \sigma')\)}
             \caption{Basic Operations\apt{This was absolutely full of problems. Please triple-check my changes.
             And do we really need two-stage calculation in the call rule, or was that just to avoid type-setting issues?}}
  \label{fig:basicops}
\end{figure}
...
\noindent
When \(d = 0\), the trace will always be infinite\apt{why? remind us about machine not halting (if that is the reason)?}; in this case we
...
fulfill this property \apt{what property?} vacuously.
...
%    & \(\textnormal{ when } (\mach,(V,\sigma)) \textnormal{ is called}\) \\
    ...
    %
    \rowcolor{black!20}
    & & & \(\textnormal{ \hspace{0.8in}} \land \mach''[\PCname] = \mach[\PCname]+4\) & \\
    %
    \rowcolor{black!10}
    2
...
%    & \(\textnormal{ when } (\mach,(V,\sigma)) \textnormal{ is (tail)called}\) \\
   ...
   \rowcolor{black!20}
    ...
    \mach' \parallel \bar{\Diamond}(\mach,\nach,\mach',\nach')\) & \\
  \end{tabular}
  \caption{Properties\apt{The hardwired +4 is a little sad, but oh well.}}
  \label{tab:props}
\end{table*}
...
A C++-style call-by-reference argument % where the reference is
passed in \(\reg\)
is instead specified by \((\reg, 0, \mathit{sz})\), which allows
the reference to be passed further down the stack.\apt{Last sentence is completely mysterious.}
...
\begin{figure*}
  ...
  \begin{subfigure}{0.6\textwidth}
    ...
    \label{sfig:stkargs}
  \end{subfigure}
  \caption{Operations supporting tail calls and argument passing on stack.\apt{This needs similar edits to the previous operations figure. In particular, same question about the $V''$ in call and tailcall.}}
  \label{fig:advops}
\end{figure*}
...
Existing operations are extended to preserve the value of \(\rho\), and the new operations
work as follows\apt{Put these and tweaked call rule into a separate figure in the same style as the existing two}:
...
and the {\em Lazy Per-Activation Tagging and Clearing} micro-policies.\apt{Reads
  awkwardly coming just after last section.}
...
\end{tabular}
\vspace*{1em}
\caption{MTTF for finding bugs in erroneous policy enforcement mechanisms\apt{what does the horizontal line mean?}}
\vspace*{-2em}
\label{tab:bug-table}
\end{table}
...
not really stack safe, and to validate a possible fix.\apt{This paragraph
  comes as a bit of a non-sequitur here.}
...
feedback during the writing process, and A\"ina Linn Georges for her enthusiastic reception
of our early work.\apt{would consider rephrasing that last.}
...
and by Deutsche Forschungsgemeinschaft (DFG, German Research Foundation) as part of the Excellence Strategy of the German Federal and State Governments, EXC 2092 CASA -- 390781972.\apt{Benjamin?}{Leo?}
