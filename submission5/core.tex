...
\cref{sec:test} the testing framework itself, and
...
\Cref{fig:main} gives C code and possible corresponding compiled 32-bit RISC-V code
...
\newcommand{\freebox}[1][10px]{\tikz \filldraw[fill=blue] (0,0) rectangle (#1,#1);}
\newcommand{\pubbox}[1][10px]{\tikz \filldraw[fill=lightgray] (0,0) rectangle (#1,#1);}
\newcommand{\objbox}[1][10px]{\tikz \filldraw[fill=yellow] (0,0) rectangle (#1,#1);}
\newcommand{\sealbox}[1][10px]{\tikz \filldraw[fill=red] (0,0) rectangle (#1,#1);}

\newcommand{\textbox}[1][]{\tikz \filldraw[fill=lightgray] (0,0) rectangle (30px,30px) node[pos=.5]{#1};}
\newcommand{\numbox}[1][]{\tikz \filldraw[fill=lightgray] (0,0) rectangle (20px,20px) node[pos=.5]{#1};}
\newcommand{\leftbox}[1][]{\tikz \filldraw[fill=blue!20] (0,0) rectangle (20px,20px) node[pos=.5]{#1};}
\newcommand{\rightbox}[1][]{\tikz \filldraw[fill=orange!20] (0,0) rectangle (20px,20px) node[pos=.5]{#1};}
...
\begin{figure}
  ...
  \begin{subfigure}{\columnwidth}
    ...
    \(\textnormal{\large \dots} \pubbox[30px] \!\! \stackrel{\textsc{\normalsize sp}}{\textbox[\tt res]}
    \stackrel{\textsc{\normalsize 4(sp)}}{\textbox[\tt sens]}
    \stackrel{\textsc{\normalsize 8(sp)}}{\textbox[\tt sec]}
    \stackrel{\textsc{\normalsize 12(sp)} \hfill}{\textbox[\(\mathtt{ra_1}\)]\textbox[\(\mathtt{ra_2}\)]}
    \textnormal{\large \dots}\)
  \end{subfigure}
...
\begin{tabular}{| l | l | l | l | l |}
  ...
  \hline
  {\tt res} & {\tt sens} & {\tt sec} & \({\mathtt ra_1}\) & \({\mathtt ra_2}\) \\
  \hline
\end{tabular}
\end{center}
...
architecture and ABI, (2) the set of observable events, and (3) a notion of value compatibility.
...
by a criterion that must hold ``for all call targets \(s\).'' In the case of \(\wbcf\),
we also need to know about the callee, so we refer to the ``call step \(s \stepstounder{} s'\).
...
and whose program counter is at the next instruction. We enjoy \(\wbcf\) if
\(\ret\) holds on return from \(\mach'\).

\paragraph*{\(\clri\)}
When our call target is \((\mach,(V,\sigma))\), we define the predicate \(\intProp\) to hold
on states \(\mach'\) if any elements that are both sealed in \(V\) and in the difference
set between \(\mach\) and \(\mach'\) are irrelevant. We enjoy \(\clri\) if \(\intProp\)
holds on return from \(\mach\).

\paragraph*{\(\clrc\)}
When our call target \((\mach,(V,\sigma))\), we begin by taking an arbitrary \(\nach\)
that is a \(\components\)-variant of \(\mach\), where \(\components\) is the set of sealed elements
in \(V\). We require that two clauses hold. On line a, the behavior of a trace from
\((\mach,(V,\sigma))\) up to its return must match that of \((\nach,(V,\sigma))\).
On line b, we define a relation \(\confProp\) that relates states \(\mach'\) and \(\nach'\)
if their corrupted set (relative to \(\mach\) and \(\nach\)) is irrelevant, and require
that it hold on the returns from \((\mach,(V,\sigma))\) and \((\nach,(V,\sigma))\).
We enjoy \(\clrc\) if both clauses hold.
...
should be kept secret, which is to say, irrelevant. Similar to \(\clri\), given a call target
\((\mach,(V,\sigma))\), we define a predicate \(\cconfProp\) to hold
on states \(\mach'\) if the difference set between \(\mach\) and \(\mach'\), that is
not \(\object\) or \(\public\), is irrelevant. If \(\cconfProp\) holds on return,
we enjoy \(\clec\).
...
just like the caller's secrets would under \(\clrc\). So, for a call target
...
%In this second case, the reference can be passed further down the stack, but otherwise
%only the immediate callee gains access. APT: 

On the other hand, if the address of an object is taken directly and passed it as a
pointer, we have no way obvious way to limit the scope of such sharing.
We treat this scenario as the caller allocating a ``public''
object that receives no protection. We extend the \(\mathbf{alloc}\) operation
...
than \(\sealed\) to express this.

\section{Enforcement}

We implement and test two micro-policies from Roessler and DeHon \cite{}:
...
tagged \(\tagStackDepth{n}\), and when it is active, the tag on the
program counter (PC tag) is \(\tagPCDepth{n}\). The tag \(\tagNoDepth\)
is for stack locations that are currently unused.

In \(DI\), \(n\) always corresponds to the depth of the stack when
the function is called. A functions must initialize its entire frame on a call
in order to tag it, and then clear it on a return.
...
as the current {\PCname} tag, and store operations also when the
target memory is \(\tagNoDepth\).

In \(LTC\), rather than initialize the entire frame,
a function will initialize each location's tag when it writes to it. It does
...
On a load, the micro-policy failstops if the value that it would load
is tagged \(\tagStackDepth{n}\) or \(\tagNoDepth\) and the PC tag doesn't
match \(n\).
...
the stack and adjusting the stack pointer. A blessed sequence also guarantees
that the full sequence executes from the beginning---no jumping into the middle.

\paragraph{Protecting Registers}

Roessler and DeHon do not protect registers, but we consider registers to be
an extension of the stack. In particular, \(\clri\) requires callee-saved
