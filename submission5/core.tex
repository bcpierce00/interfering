...
\begin{figure}
  ...
  \judgmentbr{\(\components = \mathit{range} ~ \SP ~ \mathit{off} ~ \mathit{sz} ~ \mach \cap \unsealed(V)\)}
    ...
    \judgment{\(V' = V \left\llbracket
      ...
      \end{cases}\right\rrbracket\)}
             {\(Op ~ \mach ~ (\mathbf{call} ~ \addr_{target} ~ \overline{\reg_{args}})
               ~ (V,\sigma) = (V',V::\sigma)\)}
    %
    \judgment{}
             {\(Op ~ \mach ~ \mathbf{return} ~ (\_, (V,\sigma')) = (V, \sigma')\)}
             \caption{Basic Operations\apt{...Do we really need two-stage calculation in the call rule, or was that just to avoid type-setting issues?}\sna{The latter. I took a stab at trying to avoid it, but I'm ambivalent about  that ``cases'' style here.}}
  \label{fig:basicops}
\end{figure}
...
%    & \(\textnormal{ when } (\mach,(V,\sigma)) \textnormal{ is called}\) \\
    ...
    %
    \rowcolor{black!20}
    & & & \(\textnormal{ \hspace{0.8in}} \land \mach''[\PCname] = \mach[\PCname]+\mathit{sz}\) & where \(\mathit{sz}\) is the size of one instruction \\
    %
    \rowcolor{black!10}
    2
  ...
  \begin{subfigure}{0.4\textwidth}
    ...
    \judgmentbr{\(b = \mach[\SP] + \mathit{off}\)}
               {\(V' = V \llbracket \addr \mapsto \unsealed |
                 b \leq a < b+\mathit{sz} \land \addr \in \object(V) \rrbracket\)}
               {\(Op ~ \mach ~ (\mathbf{dealloc} ~ \mathit{off, sz}) ~ (V,\sigma) = (V',\sigma)\)}
  ...
  \begin{subfigure}{0.6\textwidth}
    ...
    \judgment{\(V' = V \left\llbracket
      ...
      \begin{cases}
        ...
        & k \in \object(V) - \mathit{passed} ~ \overline{sa} ~ \mach \\
        V(k) & \textnormal{otherwise} \\
      \end{cases}\right\rrbracket\)}
  ...
  \begin{minipage}{1\textwidth}
    ...
    \judgment{\(V' = V \left\llbracket
      ...
      \begin{cases}
        \unsealed & \textnormal{if } k \in CLR \\
        \public & \textnormal{if } k \in \overline{\reg_{\mathit{args}}} \\
        \sealed & \textnormal{if } k \in \WORDS \textnormal{ and } k \in \object(V) - (\mathit{passed} ~ \overline{sa} ~ \mach \cup \mathit{capped} ~ \overline{\reg_{args}} ~ \rho) \\
        V(k) & \textnormal{otherwise} \\
      \end{cases}\right\rrbracket\)}
             {\(Op ~ \mach ~ \psi ~ (V,\sigma,\rho) = (V',V::\sigma,\rho)\)}
...
and by NSF award \#2145649, CAREER: Fuzzing Formal Specifications.
\apt{Benjamin?}
