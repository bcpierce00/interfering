...
with the exception of Skorstengaard and Georges \rb{(refs, and add a ``but'' here that motivates our development!)}. Others are fundamentally ill-suited for
formalization, as they aim to impede attackers but could never provide universal protection
...
an \rb{(flexible? ``general''?)} enforcement mechanism that does not yet have one.
...
Our target for validation is the {\em Lazy Tagging and Clearing} (\(LTC\)) micro-policy of Roessler
and DeHon \cite{DBLP:conf/sp/RoesslerD18}. \(LTC\) is peculiar in that function activations
may write into one another's stack frames, as long as the data cannot be accessed afterward.
It is therefore too weak to be characterized by the state-of-the-art properties from
Georges et al. \rb{(Do we explain why anywhere?)} We give a characterization that does apply to it, and expand the features that
it supports \rb{(try to be a bit more explicit?)}.
...
\rb{(Really, the abstraction makes the problem largely if not completely transparent at the language level: these are low-level attacks that break that abstraction)}
...
We are not attempting to create \rb{(``to present''?)} a universal definition of stack safety for all systems.
Rather, we define the stack safety of a single, fairly realistic system \rb{(if exotic capability machines count as realistic, this is no less so than those)}, decomposed into
